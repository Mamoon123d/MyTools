...
public class BlurLayout extends FrameLayout {
    ...
    public BlurLayout(Context context, AttributeSet attrs) {
        ...
        if (!isInEditMode()) {
            ImageM.BlurOption.init(context);
        }

        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.BlurLayout,
                0, 0);

        try {
            mDownscaleFactor = a.getFloat(R.styleable.BlurLayout_blur_downscaleFactor, DEFAULT_DOWNSCALE_FACTOR);
            mBlurRadius = a.getInteger(R.styleable.BlurLayout_blur_blurRadius, DEFAULT_BLUR_RADIUS);
            mFPS = a.getInteger(R.styleable.BlurLayout_blur_fps, DEFAULT_FPS);
            mCornerRadius = a.getDimension(R.styleable.BlurLayout_blur_cornerRadius, DEFAULT_CORNER_RADIUS);
    ...
    private Bitmap blur() {
        ...
        if (!mViewLocked) {
            // Blur the bitmap.
            bitmap = ImageM.BlurOption.getInstance().blur(bitmap, mBlurRadius);
     ...
     */
    private Bitmap getDownscaledBitmapForView(View view, Rect crop, float downscaleFactor) throws BlurOptionException, NullPointerException {
        ...
        if (screenView.getWidth() <= 0 || screenView.getHeight() <= 0 || width <= 0 || height <= 0) {
            throw new BlurOptionException("No screen available (width or height = 0)");
        }
    ...
    public void lockView() {
        ...
        if (mActivityView != null && mActivityView.get() != null) {
            ...
            try {
                ...
                mLockedBitmap = ImageM.BlurOption.getInstance().blur(mLockedBitmap, mBlurRadius);
            } catch (Exception e) {
                // ignore
            }
        }
